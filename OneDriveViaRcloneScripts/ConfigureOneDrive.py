...
# Create bat and vbs files if they do not exist
if not os.path.exists("OneDriveViaRcloneScripts"):
    os.makedirs("OneDriveViaRcloneScripts")

if not os.path.exists("OneDriveViaRcloneScripts/RunVbs.bat"):
    open("OneDriveViaRcloneScripts/RunVbs.bat", "w").close()
if not os.path.exists("Scripts/MountOneDrive.vbs"):
    open("OneDriveViaRcloneScripts/MountOneDrive.vbs", "w").close()
if not os.path.exists("Scripts/MountOneDrive.bat"):
    open("OneDriveViaRcloneScripts/MountOneDrive.bat", "w").close()

# Get absolute paths
runvbspath = os.path.abspath("OneDriveViaRcloneScripts/RunVbs.bat")
mountvbspath = os.path.abspath("OneDriveViaRcloneScripts/MountOneDrive.vbs")
mountbatpath = os.path.abspath("OneDriveViaRcloneScripts/MountOneDrive.bat")

# Create a .bat file that runs MountOneDrive.vbs via Task scheduler
with open("OneDriveViaRcloneScripts/RunVbs.bat", "w") as f:
    f.write(f"""@echo off
start "" "{mountvbspath}"
""")
    
# Create a .vbs file that runs MountOneDrive.bat silently
with open("OneDriveViaRcloneScripts/MountOneDrive.vbs", "w") as f:
...
with open("OneDriveViaRcloneScripts/MountOneDrive.bat", "w") as f:
    f.write(f"""@echo off
